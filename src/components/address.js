    ...
    getLocations() {
        try {
            ...
            AsyncStorage.getItem('AddHistory', (err, store) => {
                ...
                if (store !== null) {
                    this.setState({buttonLabel: ror[0].local,myLocation: ror[0].local, address: ror[0].address})

                }
                if (this.state.myLocation && this.state.address) {
        ...
        if(!this.state.situation){
            return(
                <Button style={{marginBottom:10,borderRadius:7,backgroundColor:'#22A7F0'}} disabled={this.state.situation}
                        onPress={() => this.gozma()}>
                    <Text style={{fontFamily:'BYekan'}}>مرحله بعد (زمان تحویل)</Text>
                </Button>
            )
        }
            return(
            ...
            <Content style={{backgroundColor:'white'}}>
                ...
                <View style={{
                    ...
                    <View style={{flex: 1, marginTop: 10}}>
                        <Button style={{borderRadius:7,backgroundColor:'white',borderWidth:2,borderColor:'#34495E'}}
                                onPress={() => this.showLocals()}>
                            <Text style={{color:'#34495E',fontFamily:'BYekan'}}>{this.state.buttonLabel}</Text>
                        </Button>
                    </View>
                    <View style={{margin:10}}>

                        <Input style={{color:'#34495E',placeholderTextColor:'red',fontFamily:'BYekan',
                    ...
                    <Text style={{textAlign:'right',color:'#34495E',fontFamily:'BYekan'}}>لطفا آدرس دقیق و محله خود را در دو کادر بالا وارد نمایید</Text>
